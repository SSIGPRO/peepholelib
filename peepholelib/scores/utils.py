...
def pair_score_name(**kwargs):
    '''
    Name used by the scores which are computed for a pair of loaders (see `peepholelib.scores.dmd.DMDScore` and `peepholelib.scores.cam.CAMExpScore`). The scores of the positive test loader change for each negative loader, so they are recorded under a name which also identifies the negative loader.
