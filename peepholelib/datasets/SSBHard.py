...
class SSBHard(DatasetWrap):
    def __init__(self, **kwargs):
        '''
        SSB-Hard Near-OOD loader (val & test). Used as a Near-OOD dataset in the OpenOOD benchmark.
        Data is expected in ImageFolder format (one subdirectory per class, or a single subdirectory
        containing all images).

        Args:
            path (str): Path to the SSB-Hard folder (ImageFolder-compatible layout).
    ...
    def __load_data__(self):
        ...
        self.__dataset__ = {
            'SSBHard-val': val_dataset,
            'SSBHard-test': test_dataset,
        }
