...
class OpenImageOCustom(ImageFolder):
...
class OpenImageO(DatasetWrap):
    def __init__(self, **kwargs):
        '''
        OpenImage-O Far-OOD loader (val & test). Used as a Far-OOD dataset in the OpenOOD benchmark.
        Data is expected in ImageFolder format (one subdirectory per class, or a single subdirectory
        containing all images).

        Args:
            path (str): Path to the OpenImage-O folder (ImageFolder-compatible layout).
    ...
    def __load_data__(self):
        ...
        self.__dataset__ = {
            'OpenImageO-val': val_dataset,
            'OpenImageO-test': test_dataset,
        }
