...
class Encoder2D(nn.Module):
    def __init__(self, num_sensors=8, seq_len=10, in_ch = 12, embedding_size='medium', lay3=False, kernel_size=[3,2]):
        super().__init__()
        ...
        self.in_ch = in_ch
        self.num_filter = max(8, 2*self.num_sensors)
        ...
        #print(f'self.enc_out_height{self.enc_out_height}\nself.enc_out_width{self.enc_out_width}')
        self.linear = nn.Linear(self.flattened_size, self.latent_dim)
        #print("Computed conv output shape:", self.enc_out_channels, self.enc_out_height, self.enc_out_width)
    ...
    def forward(self, x):
        ...
        #print(f'x beforee flatteninf{x.shape}')
        x = self.flatten(x)
        #print(f'x after flatteninf{x.shape}')
...
class Decoder2D(nn.Module):
    def __init__(self, num_sensors=8, seq_len=10, flattened_size=384,
                 latent_dim=128, embedding_size='medium', lay3=False,
                 kernel_size=[3,2], encoder_shape=None):
        super().__init__()
        ...
        print(f'self._enc_channels{self._enc_channels}\nself._enc_height{self._enc_height}\nself._enc_width{self._enc_width}')
...
class AE2D_instance(nn.Module):
    
    def __init__(self, num_sensors=8, seq_len=10, kernel_size=[3,2],
                 embedding_size='medium', lay3=False):
        super(AE2D_instance, self).__init__()
...
def train_AE2D_instance(train_iter, test_iter, model, criterion, 
        ...
        for epoch in tqdm(range(epochs), unit='epoch', dynamic_ncols=True):
            ...
            for i, batch in tqdm(enumerate(train_iter), total=len(train_iter), unit="batch", dynamic_ncols=True):
                ...
                #outputs = model(inputs).to(device) commented Elma
