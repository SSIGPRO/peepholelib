...
class ClassDependentKMeans(CDCBase):
    ...
    def fit(self, **kwargs):
        '''
        Fit one KMeans per class, using only data from that class.
        ...
        if verbose: print('\n ---- Class-Dependent-Classifier KMeans (one KMeans per class)\n')

        for c in range(self.nl_model):
            class_mask = labels == c
            class_data = data[class_mask]

            if verbose: print(f'Fitting KMeans for class {c} ({class_data.shape[0]} samples)')

            self._classifiers[c].fit(class_data.clone().detach())

        return

    def classifier_probabilities(self, **kwargs):
        '''
        Compute the probability of each sample under each class KMeans.
        ...
        '''
        data = kwargs['data']
        # for each class KMeans, get distances to its clusters and take the min (nearest centroid distance)
        min_distances = torch.stack(
            [self._classifiers[c].transform(data).min(dim=1).values for c in range(self.nl_model)],
            dim=1
        )  # (N, nl_model)
    ...
    def save(self, **kwargs):
        ...
        for c, clf in enumerate(self._classifiers):
            clf.save(self._clas_path / f'class_{c}')
        return

    def load(self, **kwargs):
        if self._clas_path.exists():
            self._classifiers = [
                tKMeans.load(self._clas_path / f'class_{c}')
                for c in range(self.nl_model)
            ]
            ok = True
        else:
