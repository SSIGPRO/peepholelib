...
class myFMN(AttackBase):
    ...
    def __init__(self, **kwargs):
        ...
        if self.norm not in self._NORM_TO_ATTACK:
            raise ValueError(
                f"norm must be one of {list(self._NORM_TO_ATTACK)}, got '{self.norm}'"
            )
        ...
        self.atk = self._NORM_TO_ATTACK[self.norm](
            steps=self.steps,
    ...
    def __call__(self, images, labels):
        images = images.clamp(0.0, 1.0).to(self.model.device)
        labels = labels.to(self.model.device)
        _, clipped_advs, _ = self.atk(self.fmodel, images, labels, epsilons=None)
        return clipped_advs.clamp(0.0, 1.0)
