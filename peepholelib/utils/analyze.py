...
def conceptogram_protoclass_score(**kwargs):
    ...
    for loader_n, ds_key in enumerate(loaders):
        if path is None: path = (Path.cwd()/phs.name).as_posix()+f'.{ds_key}.{score_name}.png'
        ...
        scores = (proto[labels]*cps).sum(dim=(1,2))
        scores = scores/(torch.norm(proto[labels], dim=(1,2))*torch.norm(cps, dim=(1,2)))
    ...
    for loader_n, ds_key in enumerate(loaders):
        ...
        scores_sl = (proto_sl[labels]*cps_sl).sum(dim=(1,2))
        scores_sl = scores_sl/(torch.norm(proto_sl[labels], dim=(1,2))*torch.norm(cps_sl, dim=(1,2)))
