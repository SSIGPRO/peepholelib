...
def plot_empp_posteriors(**kwargs):
    ...
    for module, driller in drillers.items():
        ...
        save_path = os.path.join(save_dir, "empp_%s.%s" % (safe_module, "jpeg"))
...
def _get_empp_coverage_scores(**kwargs):
    ...
    for module, driller in drillers.items():
       # empp = driller["_empp"]
        ...
        print(f"[{module}] Class Coverage: {n_class_represented}/{n_classes} (≥ {threshold}) — {100 * class_coverage:.2f}%")
        print(f"[{module}] Cluster Coverage: {n_cluster_active}/{n_clusters} (≥ {threshold}) — {100 * cluster_coverage:.2f}%")
