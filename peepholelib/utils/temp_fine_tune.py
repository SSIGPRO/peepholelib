...
def fine_tune(**kwargs):
    ...
    if iterations == 'full': 
        ...
        iter_train = ceil(len(corevectors._corevds[train_key])/bs)
        iter_val = ceil(len(corevectors._corevds[val_key])/bs) 
    ...
    train_dl = DataLoader(dataset=corevectors._corevds[train_key], batch_size=bs, shuffle=True, collate_fn=lambda x:x)
    val_dl = DataLoader(dataset=corevectors._corevds[val_key], batch_size=bs, shuffle=True, collate_fn=lambda x:x) 
