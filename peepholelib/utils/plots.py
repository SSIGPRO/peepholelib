...
def plot_ood(**kwargs):
    ...
    - iod_loader (list[str]): loaders to consider, usually ['train', 'test', 'val'], if 'None', gets all loaders in 'scores'. Defaults to 'None'.
    - ood_loader (list[str]): loaders to consider, usually ['train', 'test', 'val'], if 'None', gets all loaders in 'scores'. Defaults to 'None'.
    ...
    iod_loaders = kwargs.get('iod_loader', None)
...
def plot_confidence(**kwargs):
    ...
    for loader_n, ds_key in enumerate(loaders):
        ...
        for score_name in scores[ds_key].keys():
            ...
            if verbose: print(f'AUC for {ds_key} {score_name} split: {_auc:.4f}')
