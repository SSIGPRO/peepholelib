...
def new_get_coreVectors(self, **kwargs):
    self.check_uncontexted()
    
    bs = kwargs['batch_size'] if 'batch_size' in kwargs else 64
    n_threads = kwargs['n_threads'] if 'n_threads' in kwargs else 1 
    verbose = kwargs['verbose'] if 'verbose' in kwargs else False
    save_activations = kwargs['save_activations'] if 'save_activations' in kwargs else True
    ...
    for ds_key in datasets:
        ...
        if file_path.exists():
            ...
            n_samples = len(self._n_samples[ds_key])
        ...
        for mk in model._target_modules.keys(): 
            if not (mk in self._corevds[ds_key]):
                ...
                self._corevds[ds_key][mk] = MMT.empty(shape=((n_samples,)+reduction_fns['cv_shape']))
