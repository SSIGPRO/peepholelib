...
def get_clip_embeddings(self, **kwargs):
   ...
   mean = torch.tensor([0.438, 0.418, 0.377], device=device).view(1,3,1,1)
   std  = torch.tensor([0.300, 0.287, 0.294], device=device).view(1,3,1,1)
   ...
   for ds_key in datasets._dss:
      ...
      for embeds_data, ds_data in tqdm(zip(embeds_dl, ds_dl), disable=not verbose, total=len(embeds_dl)):
         ...
         with torch.no_grad():       
                imgs = torch.stack([
                    d['image'].squeeze(0) if d['image'].dim() == 4 else d['image']
                    for d in ds_data
